...
/**
 * Bare, single-select, server-side-searchable account control. Renders like any
 * other form field — NO Panel, no own heading; the surrounding `FieldShell`
 * (via the RelationshipField renderer) supplies the label/description/error
 * chrome, so this matches every other field on the page and is admin-placeable
 * through the layout editor.
 *
 * Debounced typeahead hits the existing accounts list endpoint
 * (`GET /api/crm/accounts`) using its `contains` (ILIKE) filter on the `name`
 * text field — scales to thousands without loading them all. When the query
 * matches no existing account, a "Create '<name>'" row creates inline (the
 * combined endpoint validates uniqueness and creates+links atomically).
 */
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        sp.set("sort", JSON.stringify({ fieldSlug: "name", direction: "asc" }));
