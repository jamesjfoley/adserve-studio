...
import type { PipelineEntity } from "@/lib/crm/pipeline";
...
export default async function CrmDashboardPage() {
  ...
  const config = readCrmModuleConfig(tenant.settings);
  if (!config.showPipeline) notFound();

  const readableSlugs = CRM_ENTITY_SLUGS.filter((s) =>
    permissions.has(`${s}.read`)
  );

  // Campaign-first: aggregate a pipeline section per ENABLED pipeline entity
  // the user may read. Campaign reads on `campaign.read`; opportunity on
  // `opportunity.read`.
  const pipelineEntities: PipelineEntity[] = [];
  if (config.campaigns && permissions.has("campaign.read")) {
    pipelineEntities.push("campaign");
  }
  if (config.opportunities && permissions.has("opportunity.read")) {
    pipelineEntities.push("opportunity");
  }
  const canForecast =
    config.opportunities && permissions.has("opportunity.read");
  const canLead = config.leads && permissions.has("lead.read");
  ...
  for (const section of data.pipelines) {
    const value = section.stages.reduce((sum, s) => sum + s.total, 0);
    const open = section.stages.reduce((sum, s) => sum + s.count, 0);
    const noun = section.noun;
    kpis.push({
      label:
        section.entity === "campaign" ? "Campaign value" : "Pipeline value",
      value: formatCurrency(value, LOCALE),
      sub: `${open} open ${open === 1 ? noun : `${noun}s`}`,
    });
  }
  if (data.forecast) {
    kpis.push({
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Widget 1 — Pipeline value by stage, one panel per enabled entity */}
        {data.pipelines.map((section) => {
          const max = Math.max(1, ...section.stages.map((s) => s.total));
          return (
            <Panel
              key={section.entity}
              title={section.label}
        ...
        {/* Widget 5 — Weighted revenue forecast (opportunity-only) */}
        {data.forecast ? (
